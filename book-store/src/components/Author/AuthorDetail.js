...
const AuthorDetail = () => {
  ...
  useEffect(()=>{
    const fetchHandler=async()=>{
        ...
        .then((res)=>res.data).then(data=>setInputs(data.author));
    };
    fetchHandler();
  },[id]) ;
  
  const sendRequest =async()=>{
    await axios.put(`http://localhost:8080/author/${id}`,{
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
        <Box display="flex" flexDirection="column" justifyContent={"center"} maxWidth={700} alignContent={"center"} alignSelf="auto" marginLeft={"auto"} marginRight="auto" marginTop={10} >
            ...
            <Button variant="contained" type="submit">Update Author</Button>
